        ...
        <div className="grid grid-cols-12">
          <div className="col-span-6 col-start-3">
            <div className=''>
              ...
              <img style={{width:300, margin:'15px'}} src={filmdetail.hinhAnh} alt={filmdetail.tenPhim} />
              </div>
              <div className='flex items-center'>
                <div className='ml-3 text-white text-lg'>
                  <p className='text-3xl font-bold'>{filmdetail.tenPhim}</p>
                  <p>Ngày khởi chiếu: <span className='text-yellow-300'>{moment(filmdetail.ngayKhoiChieu).format('DD-MM-YYYY hh:mm A')}</span></p>
                  <p>Mô tả phim: <span className='text-gray-400 text-s'>{filmdetail.moTa}</span></p>
                  <p>Đánh giá từ khán giả: <span className='text-yellow-300'>{filmdetail.danhGia}</span> </p>
                </div>
              </div>
            </div>
          </div>
          <div className='col-span-4 flex items-center justify-center flex-col'>
            <h1 className='text-2xl font-bold text-white mr-8 mt-10'>Rating</h1>
            <h1 className='text-2xl text-yellow-400 flex justify-center items-center mr-8'><Rate allowHalf value={filmdetail.danhGia / 2} style={{ fontSize: 30 }} /></h1>
            <div className={`c100 p${(filmdetail.danhGia) * 10} big`}>
              <span className='text-yellow-300 text-xl'>{(filmdetail.danhGia) * 10}%</span>
            </div>
          </div>
        </div>
                        ...
                        return <div className="mt-5" key={index}>
                          ...
                          <div className="grid grid-cols-4">
                            {cumRap.lichChieuPhim?.slice(0, 12).map((lichChieu, index) => {
                              return <NavLink to={`/checkout/${lichChieu.maLichChieu}`} key={index} className="col-span-1 font-bold text-blue-600">
